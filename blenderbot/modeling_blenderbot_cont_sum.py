...
# Copied from transformers.models.bart.modeling_bart.BartAttention with Bart->Blenderbot
class BlenderbotPALsAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
    ...
    def forward(
        ...
        attn_output = attn_output.view(bsz, self.num_heads, tgt_len, self.head_dim)
        attn_output = attn_output.transpose(1, 2)
        attn_output = attn_output.reshape(bsz, tgt_len, self.hidden_dim)
...
class PalsLite(nn.Module):
    def __init__(self, config: BlenderbotConfig, embed_dim: int, hidden_dim: int, num_heads: int):
        super(PalsLite, self).__init__()
        self.self_attn = BlenderbotPALsAttention(
            embed_dim=embed_dim,
...
class AttnBlend(nn.Module):
    def __init__(self, config: BlenderbotConfig, embed_dim: int, out_dim: int, bias: bool = True):
        super(AttnBlend, self).__init__()
        self.k_proj = nn.Linear(embed_dim, embed_dim, bias=bias)
        self.v_proj = nn.Linear(embed_dim, embed_dim, bias=bias)
        self.q_proj = nn.Linear(embed_dim, embed_dim, bias=bias)
    ...
    def forward(self, hidden_states):
        ...
        bs, seq_len, num_branches, dim = key_states.size()
        query_states = query_states.reshape(bs*seq_len, num_branches, dim)
        key_states = key_states.reshape(bs*seq_len, num_branches, dim)
        value_states = value_states.reshape(bs*seq_len, num_branches, dim)
        attn_weights = torch.bmm(query_states, key_states.transpose(1, 2))
        attn_weights = nn.functional.softmax(attn_weights, dim=-1)
        attn_output = torch.bmm(attn_weights, value_states)
        cont_output = attn_output[:, 0, :]
        cont_output = cont_output.squeeze(1).view(bs, seq_len, dim)
        output = self.out_proj(cont_output)
        return output


class BlenderbotDecoderPALsLayer(nn.Module):
    def __init__(self, config: BlenderbotConfig):
        super().__init__()
        self.embed_dim = config.d_model
        ...
        multi = PalsLite(config, config.d_model, int(config.d_model / 2), config.pals_attention_heads)
        self.branches = nn.ModuleList([nn.ModuleList([copy.deepcopy(multi) for _ in range(branch_population)])
                                       for branch_population in config.branch_structure])
        self.blend_layer = nn.Linear(config.d_model, config.d_model)
        self.encoder_attn_layer_norm = nn.LayerNorm(self.embed_dim)
        self.fc1 = nn.Linear(self.embed_dim, config.decoder_ffn_dim)
        self.fc2 = nn.Linear(config.decoder_ffn_dim, self.embed_dim)
        self.final_layer_norm = nn.LayerNorm(self.embed_dim)
    ...
    def set_working_branches(self, working_branches):
        self.working_branches = working_branches
    ...
    def forward(
        ...
        # Self Attention
        # decoder uni-directional self-attention cached key/values tuple is at positions 1,2
        present_key_value = past_key_value[:2] if past_key_value is not None else None
        # add present self-attn cache to positions 1,2 of present_key_value tuple
        working_branches = [dialog_acts[0], sentiments[0]]
        pals_output = [branch[worker_idx](hidden_states, attention_mask=attention_mask) for branch, worker_idx in
                          zip(self.branches, working_branches)]
        
        self_attn_past_key_value = past_key_value[:2] if past_key_value is not None else None
        content_branch, self_attn_weights, present_key_value = self.self_attn(
            hidden_states=hidden_states,
            past_key_value=self_attn_past_key_value,
            attention_mask=attention_mask,
            layer_head_mask=layer_head_mask,
        ...
        pals_concat = torch.cat(pals_output, dim=-1)
        blended_states = self.blend_layer(pals_concat)
        hidden_states = content_branch + blended_states
...
class BlenderbotDecoder(BlenderbotPreTrainedModel):
    ...
    def __init__(self, config: BlenderbotConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([BlenderbotDecoderLayer(config) for _ in range(config.decoder_layers - config.num_pal_layers)] + \
                                    [BlenderbotDecoderPALsLayer(config) for _ in range(config.num_pal_layers)])
        self.layer_norm = nn.LayerNorm(config.d_model)
    ...
    def set_working_branches(self, working_branches):
        for layer in self.layers[-self.config.num_pal_layers:]:
            layer.set_working_branches(working_branches)
...
class BlenderbotModel(BlenderbotPreTrainedModel):
    ...
    def set_working_branches(self, working_branches):
        self.decoder.set_working_branches(working_branches)
...
class BlenderbotForConditionalGeneration(BlenderbotPreTrainedModel):
    ...
    def __init__(self, config: BlenderbotConfig, return_ppl: bool = False):
        super().__init__(config)
        self.model = BlenderbotModel(config)
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))
        self.lm_head = nn.Linear(config.d_model, self.model.shared.num_embeddings, bias=False)
        self.return_ppl = return_ppl
    ...
    def set_working_branches(self, working_branches):
        self.model.set_working_branches(working_branches)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            loss1 = masked_lm_loss.view(labels.size(0), labels.size(1))
            label_size = torch.sum(labels != 0, dim=1).type(loss1.type())
